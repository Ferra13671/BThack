...
public class HighwayBuilder extends Module {
    ...
    public HighwayBuilder() {
        ...
        buildTicks = new NumberSetting("BuildTicks", this, 1, 0, 5, true, () -> !mode.getValue().equals("Tunnel"));
        borders = new BooleanSetting("Borders", this, true, () -> !mode.getValue().equals("Tunnel"));
        extraBlocks = new BooleanSetting("ExtraBlocks", this, false, () -> !mode.getValue().equals("Tunnel"));
        ...
        clearFloat = new BooleanSetting("ClearFloat", this, true, () -> !mode.getValue().equals("Tunnel"));
        clearBorder = new BooleanSetting("ClearBorder", this, true, () -> !mode.getValue().equals("Tunnel"));
        clearEBlocks = new BooleanSetting("ClearEBlocks", this, true, () -> !mode.getValue().equals("Tunnel"));
    ...
    private void breakAction(Thread thread, byte[] moveFactor) {
        ...
        breakInternal(thread, moveFactor, schematic);
        if (mode.getValue().equals("Tunnel")) {
            breakInternal(thread, new byte[]{(byte) (moveFactor[0] + 1), (byte) (moveFactor[1] + 1)}, schematic);
            breakInternal(thread, new byte[]{(byte) (moveFactor[0] - 1), (byte) (moveFactor[1] - 1)}, schematic);
        }
    }

    private void breakInternal(Thread thread, byte[] moveFactor, ArrayList<Vec3i> schematic) {
        ...
        destroyThread.setIgnoreBlocks(Arrays.asList(Blocks.OBSIDIAN, Blocks.CRYING_OBSIDIAN));
        destroyThread.start();
        try {
            thread.sleep(2);
        } catch (Exception ignored) {}
    ...
    private void gotoAction(Thread thread, byte[] moveFactor) {
        Goto gotoN = new Goto(mc.player.getX() + (moveFactor[0] * 0.16), mc.player.getZ() + (moveFactor[1] * 0.16), CollisionAction.NONE);
        gotoN.start();
        try {
            thread.sleep(2);
        } catch (Exception ignored) {}
