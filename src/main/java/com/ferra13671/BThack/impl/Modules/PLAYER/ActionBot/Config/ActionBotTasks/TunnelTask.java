...
public class TunnelTask extends ActionBotTask {
    ...
    private boolean checkLava() {
        ...
        for (BlockPos pos : blockPosData) {
            if (lavaData.contains(mc.world.getBlockState(pos).getBlock())) {
                return true;
            } else if (lavaData.contains(mc.world.getBlockState(new ModifyBlockPos(pos.getX(), pos.getY() - 1, pos.getZ())).getBlock())) {
                return true;
            } else if (lavaData.contains(mc.world.getBlockState(new ModifyBlockPos(pos.getX(), pos.getY() + 1, pos.getZ())).getBlock())) {
                return true;
            } else if (lavaData.contains(mc.world.getBlockState(new ModifyBlockPos(pos.getX(), pos.getY() + 2, pos.getZ())).getBlock())) {
                return true;
            }
        }
        return false;
    ...
    private final Set<Block> lavaData = Sets.newHashSet(
            Blocks.LAVA,
